---
layout: page
permalink: /team/
title: team
description: 我的团队与实验室
nav: true
nav_order: 8
---

<div style="display: flex; justify-content: center; align-items: center; min-height: 60vh;">
  <a href="https://ekcsu.team" target="_blank" style="text-decoration: none;">
    <div style="
      background-color: var(--global-card-bg-color);
      border: 2px solid var(--global-theme-color);
      border-radius: 16px;
      padding: 40px;
      width: 100%;
      max-width: 380px;
      text-align: center;
      box-shadow: 0 8px 24px rgba(0, 0, 0, 0.12);
      transition: all 0.3s ease;
      cursor: pointer;
    " 
    onmouseover="this.style.boxShadow='0 12px 32px rgba(0, 0, 0, 0.18)'; this.style.transform='translateY(-4px)';"
    onmouseout="this.style.boxShadow='0 8px 24px rgba(0, 0, 0, 0.12)'; this.style.transform='translateY(0)';">
      
      <!-- 队徽图片（替换为你的队徽路径） -->
      <img src="/assets/img/EK_black.png" alt="团队队徽" style="width: 120px; height: 120px; margin-bottom: 24px; object-fit: contain;">
      
      <!-- 团队名称 -->
      <h2 style="margin: 0 0 12px 0; color: var(--global-theme-color); font-size: 1.8rem; font-weight: 700;">
        中南大学EK机器人队
      </h2>
      
      <!-- 描述 -->
      <p style="margin: 0 0 20px 0; color: var(--global-text-color-light); font-size: 1rem; line-height: 1.6;">
        成立于 2020 年 10 月，由原 EUT 战队、KK 机器人战队、原航空航天学院航模协会合并而成。聚焦机器人竞赛与工程创新。
      </p>
      
      <!-- 链接提示 -->
      <div style="color: var(--global-theme-color); font-weight: 600; font-size: 0.95rem; display: flex; align-items: center; justify-content: center; gap: 8px;">
        <span>访问官网</span>
        <i class="fa-solid fa-arrow-up-right" style="font-size: 0.8rem;"></i>
      </div>
    </div>
  </a>
</div>